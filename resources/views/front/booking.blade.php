...
@section('title','Booking | travela')
    
@section('content')
    <nav class="mt-8 px-4 w-full flex items-center justify-between">
        <a href="{{ route('front.detail', $packageTour->slug)}}">
        <img src="{{ asset('assets/icons/back.png')}}" alt="back">
        </a>
        <p class="text-center m-auto font-semibold">Booking</p>
        <div class="w-12"></div>
    </nav>
    <form action="{{ route('front.booking_store', $packageTour->slug)}}" enctype="multipart/form-data" method="POST" class="flex flex-col gap-8">
        @csrf
        ...
        <div class="bg-white p-4 rounded-[26px] flex items-center gap-3">
            ...
            <div class="flex flex-col gap-1">
                <p class="font-semibold text-sm tracking-035 leading-[22px]">{{ preg_replace('/((?:\S+\s+){2})/', '$1' . PHP_EOL, $packageTour->name) }}</p>
        ...
        <div class="navigation-bar fixed bottom-0 z-50 max-w-[640px] w-full h-[85px] bg-black rounded-t-[25px] flex items-center justify-between px-6 shadow-[-6px_-8px_20px_0_#00000008]">
            ...
            @elseif ($bookingCheck['start_date'] >= Carbon\Carbon::now())
                <a href="{{ route('front.choose_bank', $bookingCheck->id)}}" class="p-[16px_24px] rounded-xl bg-blue w-fit text-white hover:bg-[#06C755] transition-all duration-300">Go to Payment</a>
            @else
                <button type="submit" class="p-[16px_24px] rounded-xl bg-blue w-fit text-white hover:bg-[#06C755] transition-all duration-300">Booking</button>
            @endif
        </div>
