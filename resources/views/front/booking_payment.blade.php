...
@section('content')
    ...
    >
      <nav class="mt-8 px-4 w-full flex items-center justify-between">
        <a href="{{ route('front.choose_bank', $packageBooking->id)}}">
          <img src="{{ asset('assets/icons/back.png') }}" alt="back" />
        </a>
        <p class="text-center m-auto font-semibold">Payment</p>
        <div class="w-12"></div>
      </nav>
      <form action="{{ route('front.booking_payment_store', $packageBooking->id)}}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-8">
        @method('PATCH')
        @csrf
        <div class="flex flex-col gap-3 px-4">
          ...
          <div class="bg-white p-4 rounded-[26px] flex items-center gap-3">
            ...
            >
              <img
                src="{{ Storage::url($packageBooking->tour->thumbnail) }}"
            ...
            <div class="flex flex-col gap-1">
              ...
              <div class="flex gap-1 items-center">
                <div class="w-4 h-4">
                  <img
                    src="{{ asset('assets/icons/calendar-grey.svg') }}"
                    class="w-4 h-4"
                    alt="icon"
                  />
                </div>
                <span class="text-darkGrey text-sm tracking-035 leading-[22px]"
                  >{{ Carbon\Carbon::parse($packageBooking->start_date)->format('d M Y')}} - {{ Carbon\Carbon::parse($packageBooking->end_date)->format('d M Y')}}</span
                >
              </div>
            </div>
          </div>
        ...
        <div class="flex flex-col gap-3 px-4">
          ...
          <div
            class="bg-white p-[16px_24px] rounded-[26px] flex flex-col gap-4"
          >
            <div class="flex flex-col gap-1">
              ...
              <div class="flex items-center gap-2">
                <div class="w-[35px] h-[25px] flex shrink-0 overflow-hidden">
                  <img
                    src="{{ Storage::url($packageBooking->bank->logo) }}"
                    class="w-full h-full object-contain object-center"
                    alt="logo"
                  />
                </div>
                <span class="text-sm tracking-035 leading-[22px]"
                  >{{ $packageBooking->bank->bank_name }}</span
                >
              </div>
            </div>
            <div class="flex flex-col gap-1">
              ...
              <div class="flex items-center gap-2">
                <div class="w-6 h-6 flex shrink-0 overflow-hidden">
                  <img
                    src="{{ asset('assets/icons/bank.svg') }}"
                    class="w-full h-full object-contain object-center"
                    alt="logo"
                  />
                </div>
                <span class="text-sm tracking-035 leading-[22px]"
                  >{{ $packageBooking->bank->bank_account_name }}</span
                >
              </div>
            </div>
            <div class="flex flex-col gap-1">
              ...
              <div class="flex items-center gap-2">
                <div class="w-6 h-6 flex shrink-0 overflow-hidden">
                  <img
                    src="{{ asset('assets/icons/moneys.svg') }}"
                    class="w-full h-full object-contain object-center"
                    alt="logo"
                  />
                </div>
                <span
                  id="bank-number"
                  class="text-sm tracking-035 leading-[22px]"
                  data-value="{{ $packageBooking->bank->bank_account_number}}"
                  >{{ $packageBooking->bank->bank_account_number }}</span
                >
                <button
                  type="button"
                  class="font-semibold text-sm tracking-035 leading-[22px] text-blue w-fit ml-auto"
                  data-copy="bank-number"
            ...
            <div class="flex flex-col gap-1">
              ...
              <div class="flex items-center justify-between">
                <span
                  id="total-payment"
                  class="font-semibold text-lg tracking-[0.6px] leading-[26px]"
                  data-value="{{ $packageBooking->total_amount }}"
                  >Rp {{ number_format($packageBooking->total_amount, 0, ',','.')}}</span
                >
                <button
                  type="button"
                  class="font-semibold text-sm tracking-035 leading-[22px] text-blue w-fit ml-auto"
                  data-copy="total-payment"
                  onclick="copyText(this)"
                >
                  Copy
                </button>
        ...
        <div class="flex flex-col gap-3 px-4">
          <div class="flex flex-col gap-1">
            ...
            <p class="text-xs leading-[20px] tracking-035 text-darkGrey">
          ...
          <div class="flex items-center gap-3">
            ...
            >
              <p
                id="placeholder"
                class="text-nowrap text-darkGrey text-sm tracking-035 leading-[22px] text-left"
              >
                Upload transfer proof
              </p>
              <div
                id="file-info"
                class="hidden flex flex-row flex-nowrap gap-3 items-center"
              >
                <div class="w-6 h-6 flex shrink-0">
                  <img src="{{ asset('assets/icons/gallery.svg') }}" alt="icon" />
                </div>
                <span id="fileName" class="text-sm text-nowrap"
                  >victoria_watson_transfer</span
                >
              </div>
            </button>
          ...
          >
            <img
              src="{{ asset('assets/backgrounds/reward-left.png') }}"
              class="object-contain h-full"
              alt="rewards"
            />
            <div class="flex flex-col -ml-[38px] relative z-10">
              <p class="text-sm leading-[22px] tracking-035 text-white">
                Claim Your Reward
              </p>
              <p class="text-xs leading-[22px] tracking-035 text-white">
                Checkout today and get reward!
              </p>
            </div>
            <img
              src="{{ asset('assets/backgrounds/reward-right.png') }}"
              class="absolute w-[73px] h-[53px] top-0 right-0"
              alt="rewards"
            />
          </div>
        </div>
